...
def provide_webpush() -> WebPush:
    ...
    webpush = WebPush(
        public_key=Path(os.getenv("VAPID_PUBLIC_KEY", "")) if env_is_file else os.getenv("VAPID_PUBLIC_KEY", "").encode(),
        private_key=Path(os.getenv("VAPID_PRIVATE_KEY", "")) if env_is_file else os.getenv("VAPID_PRIVATE_KEY", "").encode(),
