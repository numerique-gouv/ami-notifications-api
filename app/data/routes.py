...
async def get_holidays(
    ...
    # target one region per zone, to limit results (else we get all regions of each requested zone)
    zones = [
        ("Zone A", "Bordeaux"),
        ("Zone B", "Lille"),
        ("Zone C", "Versailles"),
    ...
    zones_query = " OR ".join(
        f"zones = '{zone}' AND location = '{location}'" for zone, location in zones
    )
    ...
    end_date = f"{start_date.year + 1}-09-15"

    response = httpx.get(
        f"{env.PUBLIC_API_DATA_EDUCATION_BASE_URL}{env.PUBLIC_API_DATA_EDUCATION_HOLIDAYS_ENDPOINT}",
        params={
            "where": f"end_date >= date'{start_date}' AND start_date < date'{end_date}' AND ({zones_query}) AND population IN ('-', 'Élèves')",
            "order_by": "start_date",
            "limit": 100,
        },
    )
