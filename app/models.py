...
async def get_notification_list_by_user(
    ...
    query = select(Notification).where(col(Notification.user) == user)
    if unread is not None:
        query = query.where(col(Notification.unread) == unread)
    query = query.order_by(col(Notification.date).desc())
    result = await db_session.exec(query)
    return list(result.all())
