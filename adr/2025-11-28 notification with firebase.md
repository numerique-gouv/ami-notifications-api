...
* Some project specific `AMI token` to authentify the client app or front-end when it calls the backend.
...
1. **Client App** sends `FCM Registration Token` to the backend (using the `AMI token`)
...
1. POST to FCM API notification payload using the stored `FCM Registration Token` linked to the targetted devices
...
1. **Client App** sends new `FCM Registration Token` to the backend (using the `AMI token`)
