...
describe('/initializeDataFromAPI.ts', () => {
  ...
  describe('initializeData', () => {
    ...
    test('should store notifications data to localStorage', async () => {
      // Given
      const searchParams = new URLSearchParams({})
      ...
      window.localStorage.setItem('unreadNotificationsCount', '')

      // When
      await initializeData(searchParams, userStore)

      // Then
      expect(window.localStorage.getItem('notifications')).not.toEqual('')
      expect(window.localStorage.getItem('unreadNotificationsCount')).toEqual('1')
    })
  })
})
