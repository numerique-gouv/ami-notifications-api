...
export class Partners {
  ...
  constructor(apiPartners: APIPartnersItem[] | null = null) {
    const partnersItems: PartnersItem[] = [];

    const items: APIPartnersItem[] = apiPartners || [];
...
export const buildPartners = async (): Promise<Partners> => {
  const apiPartnersItems: APIPartnersItem[] = await retrievePartners();
  return new Partners(apiPartnersItems);
};
