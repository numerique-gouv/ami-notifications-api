import type { APIAgendaItem, APIAgendaItems } from '$lib/api-agenda';
...
export class Agenda {
  ...
  constructor(agenda_items: APIAgendaItems | null = null, date: Date | null = null) {
    const today = date || new Date();
    today.setHours(0, 0, 0, 0);
    const items: Item[] = [];

    const school_holidays: APIAgendaItem[] = agenda_items?.school_holidays || [];
    const public_holidays: APIAgendaItem[] = agenda_items?.public_holidays || [];
    const elections: APIAgendaItem[] = agenda_items?.elections || [];
...
export const buildAgenda = async (date: Date | null = null): Promise<Agenda> => {
  const today = date || new Date();
  const agenda_items: APIAgendaItems = await retrieveAgenda(today);
  return new Agenda(agenda_items, today);
};
