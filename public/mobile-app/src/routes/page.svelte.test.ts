...
describe('/+page.svelte', () => {
  ...
  test('should call authorize endpoint when click on France Connect button', async () => {
    ...
    expect(globalThis.window.location.href).equal(
      'https://fcp-low.sbx.dev-franceconnect.fr/api/v2/authorize?scope=openid+given_name+family_name+preferred_username+birthdate+gender+birthplace+birthcountry+sub+email+given_name_array&redirect_uri=https%3A%2F%2Flocalhost%3A8000%2Fami-fs-test-login-callback&response_type=code&client_id=88d6fc32244b89e2617388fb111e668fec7b7383c841a08eefbd58fd11637eec&state=stateazertyuiopqsdfghjklmwxcvbn012345&nonce=nonceazertyuiopqsdfghjklmwxcvbn012345&acr_values=eidas1&prompt=login'
    )
  })
})
