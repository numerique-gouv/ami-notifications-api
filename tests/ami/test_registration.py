...
async def test_unregister(
    ...
    login(webpush_registration.user, test_client)

    all_registrations = (await db_session.execute(select(Registration))).scalars().all()
    assert len(all_registrations) == 1

    response = test_client.delete(f"/api/v1/users/registrations/{webpush_registration.id}")
...
async def test_list_registrations(
    ...
    assert response.json()[0]["user_id"] == str(webpush_registration.user_id)
