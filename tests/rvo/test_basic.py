...
async def test_rvo_login_callback_bad_nonce(
    ...
    assert (
        response.url
        == "http://testserver.local/rvo?error=Erreur+lors+de+la+FranceConnexion&error_description=Veuillez+r%C3%A9essayer+plus+tard."
    )
...
async def test_login_callback_bad_state(
    ...
    assert (
        redirected_url
        == "/rvo?error=Erreur+lors+de+la+FranceConnexion&error_description=Veuillez+r%C3%A9essayer+plus+tard."
    )
