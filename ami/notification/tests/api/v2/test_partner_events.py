...
async def test_create_webpush_event(
    ...
    assert notification2.item_parent_partner_id == "dinum-dn"
    ...
    assert event_data["content_private_body"] not in captured_push_messages[0]


@pytest.mark.django_db
def test_create_mobile_event(
    app,
    settings,
...
@pytest.mark.django_db
def test_create_event_user_does_not_exist_and_consent_is_enabled(
    app,
    partner_auth: dict[str, str],
    httpx_mock: HTTPXMock,
    monkeypatch: pytest.MonkeyPatch,
...
@pytest.mark.django_db
def test_create_event_and_does_not_create_consent_when_consent_is_disabled(
    app,
    webpush_registration: Registration,
    partner_auth: dict[str, str],
    monkeypatch: pytest.MonkeyPatch,
...
@pytest.mark.django_db
def test_create_event_and_create_consent_when_consent_is_enabled(
    app,
    webpush_registration: Registration,
    partner_auth: dict[str, str],
    monkeypatch: pytest.MonkeyPatch,
...
@freeze_time("2026-01-23 10:36:00")
@pytest.mark.django_db
def test_create_event_and_update_consent_when_consent_is_enabled(
    app,
    user: User,
    webpush_registration: Registration,
    partner_auth: dict[str, str],
...
@pytest.mark.django_db
def test_create_event_send_ko_with_400_when_required_fields_are_missing(
    app,
    partner_auth: dict[str, str],
) -> None:
    ...
    assert response.json == {
        ...
        "event_date": ["Ce champ est obligatoire."],
    }


@pytest.mark.django_db
def test_create_event_send_ko_with_400_when_required_fields_are_empty(
    app,
    partner_auth: dict[str, str],
) -> None:
    event_data = {
