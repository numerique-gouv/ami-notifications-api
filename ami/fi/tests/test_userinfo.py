...
def test_userinfo(
    ...
    response = django_app.get(
        "/api/v1/fi/userinfo/", headers={"AUTHORIZATION": f"Bearer {auth_token}"}
    )
...
def test_userinfo_missing_auth_header(
    ...
    response = django_app.get("/api/v1/fi/userinfo/", status=403)
...
def test_userinfo_wrong_format_auth_header(
    ...
    FISession.objects.create(user_data=user_data, access_token="fake-access-token")
    response = django_app.get(
        "/api/v1/fi/userinfo/", headers={"AUTHORIZATION": auth_token}, status=403
    )
