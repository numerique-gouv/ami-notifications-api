...
def test_token_invalid_data_code(
    ...
    response = app.post("/api/v1/fi/token/", token_data, status=400)
...
def test_token_invalid_redirect_uri(
    ...
    assert response.json == {
        "redirect_uri": [
            f"'redirect_uri' doit être '{settings.FI_REDIRECT_URI}', trouvé 'invalid-redirect-uri'"
        ]
    }
...
def test_token_fi_session_not_found(
    ...
    nonce = "fake-nonce"
    code = "fake-code"
    other_code_hash = make_password("other-code", settings.FI_HASH_SALT)
    FISession.objects.create(user_data=user_data, nonce=nonce, code=other_code_hash)
    token_data = {
        "code": code,
        "grant_type": "authorization_code",
        "redirect_uri": settings.FI_REDIRECT_URI,
